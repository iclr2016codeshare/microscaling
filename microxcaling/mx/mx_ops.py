...
def _quantize_mx(
    ...
    axes = [x + A.ndim if x < 0 else x for x in axes] # if x==-1, select last dimension: A.ndim - 1

    # Custom CUDA only supports limited rounding modes
    custom_cuda = custom_cuda and round in RoundingMode.string_enums()

    # fp4e2m1: e=2, m=3, emax=2 = 2**(e-1) <-- e_unbiased={2,1,0,-1}; e_biased(b=1)={3,2,1,0}={11,10,01,00}
    # fp8e5m2: e=5, m=4, emax=5
    # fp16e5m10: e=5, m=10, emax=15
    # fp32e8m23: e=8, m=23, emax=127
    # fp32e7m24: e=7, m=24, emax=127...
    ...
    if A.device.type == "cuda" and custom_cuda and len(axes) == 1 and not per_tensor and not per_channel:
        axis = axes[0]
        if block_size == 0:  # block_size 0 is per channel along selected axis
    ...
    if custom_cuda:
        ...
        assert(shared_exp_method == "max")  # !!! always 'max' when called from mx.Linear
        max_values = A.abs().max(dim=axis, keepdim=True).values  # max of each BLOCK, because A was reshaped (e.g. X = [1, 2048, 4096] --> [1, 2048, 512, 8] with gs=8)
