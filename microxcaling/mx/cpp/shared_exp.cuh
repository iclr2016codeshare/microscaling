...
int clamp_shared_exp(
    int shared_exp,  // max biased exp in block AFTER CORRECTION for element max possible exp
    const int ebits
) {
    ...
    int emax = ebits != 0 ? (1 << (ebits-1)) - 1 : FLOAT32_EXP_MAX;  // emax = 2**7-1 = 127
    int shared_ub = shared_exp - FLOAT32_EXP_BIAS;  // unbound = shared_exp - 127; example: 116 - 127 = -11
...
float mx_get_shared_scale(
    int shared_exp,  // max biased exp in block
    const int scale_bits,
    const float elem_max_norm
) {
    // Offset shared exponent by elem_emax, preserve NaNs
    // elem_emax is UNBIASED exponent of MAX REPRESENTABLE element
    // !!! what's the purpose of this shift? larger shift for elem representation with many bits
    ...
    const int scale_mant = \
            ...
            // FLOAT32_IMPLIED1 is 2^23 (24th bit from the right is 1);
            // - if shared_exp (biased) is subnormal or overflows, scale_mant = 23rd bit
            // - otherwise, scale_mant = 0
