...
void shift_right_round_mantissa(
    ...
    } else if ((rounding_mode == rd_up) || (rounding_mode == rd_up_denorm)) {
        // 1 at all truncation locations
        int mask = (1 << tbits)-1;
        // Don't need to decide tie
        sticky = (mantissa & mask); // True if the last bit before truncation location is 0
    }
    ...
    if ((rounding_mode == rd_away || rounding_mode == rd_even) && (allow_overflow || mantissa != ((1 << (mbits + 1)) - 1))) {
        if (!(tie && even))
            mantissa = mantissa + 1;
    } else if ((rounding_mode == rd_up) && (allow_overflow || mantissa != ((1 << (mbits + 1)) - 1))) {
        //Round up for all modes
        if (sticky) {
            mantissa = mantissa + 2;
        }
    } else if ((rounding_mode == rd_up_denorm) && (allow_overflow || mantissa != ((1 << (mbits + 1)) - 1))) {
        // Only round up for subnormal numbers
        if ((sticky) && (exp_diff > 0)) {
            mantissa = mantissa + 2;
        } else {
            if (!(tie && even)) {
                mantissa = mantissa + 1;
...
float quantize_elemwise(
    ...
    const int new_bias = is_int ? 1 : (1 << (exp_bits-1)) - 1;  //  2**(e-1) - 1
